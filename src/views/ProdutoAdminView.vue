...
function editar(editproduto) {
  ...
  produto.value.categoria = categorias.value.find(
    (categoria) => categoria.id === editproduto.categoria.id
  );
  produto.value.marca = marcas.value.find(
    (marca) => marca.id === editproduto.marca.id
  );
}
